...
It reads data/config.json by default, so the same script can run on The Pool Lab,
ThermaPeak, or any branch that defines domain/apexDomain there.
"""
...
def main() -> int:
    ...
    parser.add_argument("--test-deployment", action="store_true", help=f"Crawl the Pool Lab test deployment: {POOL_LAB_TEST_BASE}")
    ...
    if args.test_deployment and not args.base_url:
        args.base_url = POOL_LAB_TEST_BASE
    ...
    redirect_test_paths = ["/", "/best-of/value", "/best-of/overall"]
